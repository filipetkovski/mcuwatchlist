...
/** All checked titles, grouped by path: { [pathId]: { [titleId]: watchedAt } } */
...
/** Body: { pathId, titleIds, watched } - marks titles watched/unwatched on one path. */
export async function POST(req: NextRequest) {
  ...
  if (watched) {
    const rows = (titleIds as string[]).map((title_id) => ({ path_id: pathId, title_id }));
    const { error } = await db.from("path_progress").upsert(rows, { onConflict: "path_id,title_id", ignoreDuplicates: true });
    if (error) return NextResponse.json({ error: "Couldn't save." }, { status: 500 });
  } else {
    const { error } = await db.from("path_progress").delete().eq("path_id", pathId).in("title_id", titleIds as string[]);
    if (error) return NextResponse.json({ error: "Couldn't save." }, { status: 500 });
  }
